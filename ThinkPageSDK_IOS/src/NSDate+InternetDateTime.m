...
//  ThinkPageSDK_iOS
//
//  Created by fangshangwei on 2/21/14.
...
+ (NSDate *)dateFromInternetDateTimeString:(NSString *)dateString formatHint:(DateFormatHint)hint
{
    ...
    if (dateString)
    {
		...
		// still invalid, possibly an incomplete string only contains date?
		if (!date)
		{
			[[NSDate internetDateTimeFormatter] setDateFormat:@"yyyy-MM-dd"];
			date = [[NSDate internetDateTimeFormatter] dateFromString:dateString];
		}
    }
